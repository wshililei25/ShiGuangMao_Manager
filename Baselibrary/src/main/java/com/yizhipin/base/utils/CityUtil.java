...
import com.yizhipin.base.data.response.ProvinceBean;
...
public class CityUtil {
    ...
    private CityUtil(Context context) {
        new Thread(() -> initJsonData(context)).start();
    }
    ...
    public void showPickerView(Context context, OnCitySelectedListener onCitySelectedListener) {// 弹出选择器
        OptionsPickerView pvOptions = new OptionsPickerBuilder(context, (options1, options2, options3, v) -> {
            ...
            if (onCitySelectedListener != null)
                onCitySelectedListener.onCitySelected(opt1tx, opt2tx, opt3tx);
        })
                .setTitleText("城市选择")
                ...
                .setSubmitText(context.getResources().getString(R.string.sure))
                .build();
